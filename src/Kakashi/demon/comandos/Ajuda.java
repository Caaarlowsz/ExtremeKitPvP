...
public class Ajuda implements Listener, CommandExecutor {
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		;
		p.sendMessage(" ");
		p.sendMessage("§aInformacoes do Servidor : §bKITPVP");
		p.sendMessage("§5§l§m---------------");
		...
		p.sendMessage("§7- §4SEM FAKE DE YOUTUBER   ");
		...
		p.sendMessage("§5§l§m---------------");
		p.sendMessage("§6§l MAIS INFORMAÇOES");
		p.sendMessage("§7");
		p.sendMessage("§5§l§m---------------");
		p.sendMessage("§5§l§mFORMULARIO PARA §d§lTRIAL");
		p.sendMessage("§7");
		p.sendMessage("§5§l§m---------------");
		return false;
	}
